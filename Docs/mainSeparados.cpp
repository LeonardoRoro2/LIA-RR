...
int obtenerTerminalSintactico(token tk) {
    // final del archivo
...
void analizadorSintactico() {
    std::stack< int > pila;
    pila.push( 244 ); // Metemos $ (Fin del archivo)
    pila.push( 0 );   // Metemos PROGRAM

    nodo* actual = primero; // primer token

    while ( !pila.empty(  ) && actual != nullptr ) {

        // Saltamos los errores léxicos para que el sintáctico no explote
        if ( actual -> tk.estado >= 500 && actual -> tk.estado != 509 ) {
            actual = actual -> siguiente;
            continue;
        }

        const int X = pila.top(  ); // Lo que hay en la cima de la pila
        const int a = obtenerTerminalSintactico( actual -> tk ); // El token actual
        ...
        if ( X >= 200 || X == 244 ) {
            if ( X == a ) {
                ...
                actual = actual -> siguiente; // Avanzamos al siguiente token
            } else {
                char error[ 150 ];
        ...
        // Si X es un No Terminal (0 al 28)
            ...
            if ( const int index_prod = predicciones[ X ][ a - 200 ]; index_prod >= 550 ) { // Si la matriz indica error (casillas vacías/sync)
                ...
                // Si no es una producción que deriva a vacio (-1)
